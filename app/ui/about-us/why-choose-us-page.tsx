      ...
      <section class="py-24 bg-surface-1">
        <div class="container mx-auto px-4 max-w-6xl">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <AdvantageCard num={1} icon="ph-chart-line-up" color="blue" title="We Measure Revenue, Not Vanity Metrics" desc="Likes and impressions don't pay salaries. We track what actually matters — conversions, ROAS, CAC, and LTV. Every campaign is measured against metrics that tie directly to your business growth." />
            <AdvantageCard num={2} icon="ph-palette" color="purple" title="Strategy + Creative Under One Roof" desc="Most agencies are either strategy shops or creative shops. Bell Bee Media is both — strategy-led creative, built by people who understand both the why and the how." />
            <AdvantageCard num={3} icon="ph-eye" color="emerald" title="Radical Transparency in Reporting" desc="We send you the real numbers — even when the results are disappointing. No smoke and mirrors. Monthly reports are plain-English, honest, and actionable." />
            <AdvantageCard num={4} icon="ph-users" color="orange" title="A Dedicated Team That Knows You" desc="You get a named account manager, creative lead, and performance specialist who understand your brand, your goals, and your competitive landscape inside out." />
            <AdvantageCard num={5} icon="ph-buildings" color="indigo" title="Proven Across 14+ Industries" desc="We bring playbooks, not guesses. Our cross-industry experience means we know what works in your sector and can deliver results faster than agencies learning your market from scratch." />
            <AdvantageCard num={6} icon="ph-lock-open" color="red" title="No Lock-In Contracts" desc="We don't need long-term contracts to retain clients — our results do that. Month-to-month flexibility means we're always accountable to your expectations." />
          </div>
        </div>
      </section>
  ...
  return ({ num, icon, color, title, desc }: { num: number; icon: string; color: string; title: string; desc: string }) => (
    <div class="flex gap-6 p-8 rounded-3xl border border-surface-2 bg-surface-1 hover:shadow-xl transition-shadow group">
      <div class="flex flex-col items-center gap-2 shrink-0">
        <div class={`w-14 h-14 rounded-2xl bg-${color}-50 text-${color}-600 flex items-center justify-center text-2xl group-hover:scale-110 transition-transform`}>
          <i class={`ph-fill ${icon}`} />
        </div>
        <span class={`font-heading text-3xl font-bold text-${color}-100`}>{num < 10 ? `0${num}` : num}</span>
      </div>
      <div class="pt-1">
    ...
    <div class="p-8 rounded-3xl border border-surface-2 bg-white flex flex-col gap-6">
      <div class="flex gap-1">
        {Array.from({ length: stars }).map(() => (
          <i class="ph-fill ph-star text-yellow-400" />
        ))}
      </div>
